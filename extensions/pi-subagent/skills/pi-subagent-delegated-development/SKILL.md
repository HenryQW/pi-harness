...
Each delegation must own one concrete outcome with one focused validation story. When the runner supports filtering, prefer the exact regression test for that unit; for Node, use `node --test --test-name-pattern "exact test name" test/example.test.ts`. Keep each unit's declared validation bounded to that unit's outcome; its declared Flow validation remains authoritative for that outcome. Order declared validation from the cheapest focused check to broader required checks. Run any required broad package or cross-unit validation once after all Flow units integrate, rather than repeating it in every unit. This separate integration gate is Main-owned, not Flow validation; it must not repeat a successful Flow check. If the affected flow or scope is not yet known, perform bounded read-only discovery first. Do not pass the parent request unchanged. Choose `modelClass` according to the delegation tool's guidance. Add non-empty `review` only for an explicit judgment that automated validation cannot establish. Call `delegate_flow` with 1–8 units; the runtime always supplies the effective Implementer and supplies the Reviewer only when a unit needs review.
