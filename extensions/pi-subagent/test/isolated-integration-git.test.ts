...
test("same-file isolated candidates merge in Main-ordered integration checkout; Main moves only after exact combined-tip gates", async (t) => {
	...
	assert.deepEqual(await runtime.reconcileStage(root, integration, base, [], firstCandidate, signal), { outcome: "ready", value: "not_started" });
	...
	const pending = await runtime.reconcileStage(root, integration, base, stages, secondCandidate, signal);
	...
	assert.equal((await runtime.confirmStage(root, integration, base, stages, secondCandidate, signal)).outcome, "unknown", "uncommitted conflict is not a resolution receipt");
	...
	const resolved = await runtime.confirmStage(root, integration, base, stages, secondCandidate, signal);
	assert.equal(resolved.outcome, "ready", JSON.stringify(resolved));
	const recovered = await runtime.reconcileStage(root, integration, base, stages, secondCandidate, signal);
