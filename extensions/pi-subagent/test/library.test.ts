...
test("Role launch resolves call, Role, then Model Task routes", async (t) => {
	...
	assert.equal(valueAfter(launch.args, "--exclude-tools"), "delegate_task,delegate_flow,delegate_flow_continue,ask_question,orchestrate_execute,orchestrate_status,orchestrate_resume,orchestrate_abort,auto_dag_execute,auto_dag_status,auto_dag_resume,auto_dag_abort");
