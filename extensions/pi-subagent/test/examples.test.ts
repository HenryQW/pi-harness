...
test("bundled pi-subagent-delegated-development Skill is valid and registered", async () => {
	...
	assert.match(skill, /When the runner supports filtering, prefer the exact regression test for that unit/i);
	assert.match(skill, /node --test --test-name-pattern "exact test name" test\/example\.test\.ts/);
	assert.match(skill, /each unit's declared validation bounded to that unit's outcome/i);
	assert.match(skill, /Order declared validation from the cheapest focused check to broader required checks/i);
	assert.match(skill, /required broad package or cross-unit validation once after all Flow units integrate.*rather than repeating it in every unit/is);
	assert.match(skill, /separate integration gate.*must not repeat a successful Flow check/i);
...
test("missing config directory still returns validated built-in implementer, reviewer, and scout roles", async (t) => {
	...
	assert.match(implementer!.systemPrompt, /Before reporting Flow completion.*temporary ignored\/build artifacts.*created solely by your own development checks/i);
	assert.match(implementer!.systemPrompt, /Preserve pre-existing ignored files, user data, and requested task outputs/i);
	assert.match(implementer!.systemPrompt, /Never use broad cleanup such as `git clean -X`; target only artifacts whose ownership is known/i);
