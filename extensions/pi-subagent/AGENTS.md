...
- Main selects `modelClass` according to `extensions/model-class-policy.ts`. A direct `model` replaces only the selected route model. The route owns its exact thinking level. Reuse this policy in every delegation tool.
