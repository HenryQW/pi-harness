...
- Cleanup is non-forced. For each validation attempt, inventory ignored leaf paths immediately before validation and record evidence only for new regular files and symlinks after every command and exact committed-state inspection succeed. After no-op approval or exact-tip integration and approved branch/`HEAD` proof, recheck ignored membership and exact evidence before non-recursive unlink. Preserve all failed-attempt, pre-existing, Implementer, Reviewer, changed, non-ignored, directory, submodule, malformed, escaped, unreadable, or uncertain work for the existing dirty inspection. On failed rebase, validation/review failure, uncertain integration, dirty state, or cleanup refusal, preserve actionable recovery evidence and never delete work that cannot be proven integrated. Never use forced worktree removal or recursive directory deletion in Flow.
