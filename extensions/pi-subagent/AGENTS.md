...
- Main populates direct `model` and `thinking` only for explicit user-requested overrides; otherwise it selects only `modelClass`. Keep the model-class selection policy in `extensions/model-class-policy.ts` and reuse it in every delegation tool; do not restate it in Skills, ADRs, or `CONTEXT.md`. This is policy only: add no provenance tracking or runtime enforcement.
