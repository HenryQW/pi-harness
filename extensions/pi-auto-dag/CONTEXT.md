...
- **Workspace Identity**: Current branch, `HEAD`, real index tree, and a Git tree built from all tracked and non-ignored untracked contents through a temporary index.
...
- State uses only the v9 schema under `.context/pi-auto-dag/`. Older state is rejected.
