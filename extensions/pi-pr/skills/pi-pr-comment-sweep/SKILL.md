...
1. Use the package-relative skill path
   `extensions/pi-pr/skills/pi-pr-comment-sweep`; call its Node helper directly.
   Set optional `$PR` and require a clean tracked tree. On resume, dirt, or head
   mismatch, read [Sweep recovery](references/recovery.md) and stop unless its
   checkpoint or adoption rules pass. For a fresh sweep:

   ```bash
   SNAPSHOT=$(mktemp)
   if [ -n "${PR:-}" ]; then
     node "extensions/pi-pr/skills/pi-pr-comment-sweep/scripts/pr-feedback.mjs" target --pr "$PR"
     node "extensions/pi-pr/skills/pi-pr-comment-sweep/scripts/pr-feedback.mjs" fetch --pr "$PR" --out "$SNAPSHOT"
   else
     node "extensions/pi-pr/skills/pi-pr-comment-sweep/scripts/pr-feedback.mjs" target
     node "extensions/pi-pr/skills/pi-pr-comment-sweep/scripts/pr-feedback.mjs" fetch --out "$SNAPSHOT"
   fi
...
5. Commit accepted fixes with scoped Conventional Commit message(s). With no
   ...
   `node "extensions/pi-pr/skills/pi-pr-comment-sweep/scripts/pr-feedback.mjs" push --snapshot "$SNAPSHOT"`; helper pushes once and accepts only exact local-head confirmation.
6. Copy baseline before final fetch:

   ```bash
   FINAL_SNAPSHOT=$(mktemp)
   cp "$SNAPSHOT" "$FINAL_SNAPSHOT"
   node "extensions/pi-pr/skills/pi-pr-comment-sweep/scripts/pr-feedback.mjs" fetch --pr "$PR" --out "$FINAL_SNAPSHOT"
   ```

   Assess delta, then resolve all addressed IDs in one command, repeating flag:
   `node "extensions/pi-pr/skills/pi-pr-comment-sweep/scripts/pr-feedback.mjs" resolve --pr "$PR" --expected-head
   ...
   `node "extensions/pi-pr/skills/pi-pr-comment-sweep/scripts/pr-feedback.mjs" checks --pr "$PR"` once. Read-only
