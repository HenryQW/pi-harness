...
7. Call `refresh` with a replacement ledger that covers the complete fresh
   feedback set exactly once. Inspect new or changed items with `show` before
   supplying that ledger.
8. Call `resolve` only with addressed, unresolved parent thread IDs. Do not
   resolve a thread classified as non-actionable or blocked. Do not post replies
   unless the user asks.
9. Call `finalize` with the exact projection returned by `refresh` and the same
   chosen checks. Finalization reruns them, then reloads feedback as a later
   state guard. It succeeds only when PR linkage, content, and thread states
   still match.
