...
- Herdr labels each workspace as `<request-id>/<task-id> · #<attempt>-<short-id>` and its worker tab as `<role> · <model-class>`. Long request, task, and Role names are shortened deterministically; full correlation tokens remain internal.
