...
When enabled, pi-bark encrypts the JSON push content with a fresh 12-character IV. This includes status titles and session names. It sends `device_key`, `ciphertext`, and `iv` to the Bark server. The GCM authentication tag is appended to the ciphertext before Base64 encoding, as Bark requires.
...
| Name | Bark concept | Description | Default |
| --- | --- | --- | --- |
| `serverUrl` | Bark server | Base URL of the server that accepts the push request. | `"https://api.day.app"` |
| `deviceKey` | Device Key | Identifies the Bark App installation that receives the push notification. | Not configured |
| `encryption` | Push Encryption | Custom Encryption Key and matching Algorithm, mode, and padding. | `null` |
| `statusNotifications` | Status notifications | Global default and sparse absolute-CWD overrides. | Enabled |
...
- Invalid config files cause a visible error and remain unchanged.
