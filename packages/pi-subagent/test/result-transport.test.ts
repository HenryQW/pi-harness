...
test("keeps complete bounded model-facing content and recovery evidence", () => {
	...
	const result = formatWorkflowResult("parallel", [succeeded(1, "second evidence"), failed(0, "failure evidence", { worktreePayload: worktree })]);
	...
	assert.deepEqual(result.details.entries, [
		{ role: "role-0", status: "failed", summary: "failure evidence", worktree },
		{ role: "role-1", status: "succeeded", summary: "second evidence" },
	]);
});

test("orders details without lifecycle display fields and sums usage", () => {
	const entries = [succeeded(2, "two", { usage: usage(2) }), succeeded(0, "zero", { usage: usage(1) })];
	const result = formatWorkflowResult("parallel", entries);
	assert.deepEqual(result.details.entries.map(({ role }) => role), ["role-0", "role-2"]);
	...
	assert.equal(JSON.stringify(result.details).match(/task|model|thinking|token|duration|aborted|call:parallel/), null);
});
