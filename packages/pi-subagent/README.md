...
With `isolation: worktree`, each child runs in `<repo>/.worktrees/subagent-<id>` on branch `pi-subagent/subagent-<id>`, so parallel children never clobber each other's files. Non-git directories and repositories with no commits silently share Main's working directory; worktree setup failures in a real git repository reject the delegation instead of silently losing isolation. The directory is excluded from git status through the repository-local exclude file. The child is told to work only inside its worktree and commit to its branch; after each run the parent sees a worktree report (path, branch, commits, dirty, pruned) appended to the result — including on failures, so kept work is always locatable. Worktrees with no branch commits and a clean tree are discarded automatically; anything holding work stays for the parent to review and merge — children never merge.
