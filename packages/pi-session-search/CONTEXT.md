...
- **No `message_end` hooks**: pi emits the event before writing the JSONL entry. Sync happens lazily at tool call plus a capped fire-and-forget backfill at `session_start`; the cap bounds attempts, and unchanged failed-file fingerprints are skipped until the file changes.
