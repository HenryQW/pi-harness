...
2. Call `auto_dag_approve` only after summary. Approval requires reviewer-recorded `PASS` for current approved-form SHA-256; native confirmation binds user approval to same candidate graph ID, hash, and waves. After persistence, tool checks current branch changes, offers to stage and commit them with a user-provided message, then identifies starting Auto DAG as next step.
