...
- Pass: clean disposable child worktree at integration `HEAD`, copy checkout-local ignored tools without sharing mutable files, and reject absolute, dangling, protected-state, source-worktree-escaping, or unusable copied symlinks before execution. Require reviewer approval and system-owned gate exit code `0`, then push integration branch and open one PR.
...
Run State schema version is `2`. v4 rejects v3 in-flight Run State; finish or abort active v3 runs before upgrading. Run State records graph hash, source commit, expected integration `HEAD`, main pane, tasks, PR, health evidence, and bounded required-gate evidence. Large gate streams and active or completed gate-process journals live in separate run files until evidence is saved. Writes are atomic.
