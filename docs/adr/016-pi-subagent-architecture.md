...
- `delegate_flow` is a separate fixed package-owned workflow. One memory-only Flow creates one Unit Worktree per independent unit, runs package-shipped Implementers in parallel, then in declared order rebases in place when needed, validates, sends exact `{base, tip, patchPath}` evidence to a read-only Reviewer in that same worktree, and fast-forwards only an exact-`PASS` full reviewed OID. It supports one same-worktree repair through `delegate_flow_continue({ guidance })`; no-op units still validate and review, rebase conflicts are terminal, and non-forced cleanup refusal is a completion warning.
