...
- Model and thinking routes remain separate from Role capability. They resolve through `@henryqw/pi-task-models`; fallback is attempted only before launch, and a started child is never retried. `modelClass` selects the task-model route, and the route owns the thinking level; an optional direct `model` replaces only that route's model and must support the route's unchanged thinking level. Main selects `modelClass` according to delegation tool policy. This has no provenance tracking or runtime enforcement.
